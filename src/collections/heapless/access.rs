...
// Helper function to create a pre-populated Vec - NOT profiled
pub fn create_populated_vec(_program_id: &Pubkey) -> Vec<Pubkey, 10> {
    let mut vec: Vec<Pubkey, 10> = Vec::new();
    for i in 0..10 {
        vec.push(Pubkey::from([(i + 1) as u8; 32])).unwrap();
    }
    vec
}

// Helper: Create vec with target at first position (1 iteration to find)
pub fn create_vec_target_first(target: &Pubkey, filler: &Pubkey) -> Vec<Pubkey, 10> {
    let mut vec: Vec<Pubkey, 10> = Vec::new();
    vec.push(*target).unwrap();
    for _ in 1..10 {
        vec.push(*filler).unwrap();
    }
    vec
}

// Helper: Create vec with target at last position (10 iterations to find)
pub fn create_vec_target_last(target: &Pubkey, filler: &Pubkey) -> Vec<Pubkey, 10> {
    let mut vec: Vec<Pubkey, 10> = Vec::new();
    for _ in 0..9 {
        vec.push(*filler).unwrap();
    }
...
#[profile]
pub fn get_first_pubkey(vec: &Vec<Pubkey, 10>) -> Result<&Pubkey, ProgramError> {
    vec.get(0).ok_or(ProgramError::InvalidAccountData)
}

#[profile]
pub fn get_10th_pubkey(vec: &Vec<Pubkey, 10>) -> Result<&Pubkey, ProgramError> {
...
#[profile]
pub fn position_pubkey_1_iters(vec: &Vec<Pubkey, 10>, target: &Pubkey) -> Option<usize> {
    vec.iter().position(|x| pubkey_eq(x, target))
}

#[profile]
pub fn position_pubkey_10_iters(vec: &Vec<Pubkey, 10>, target: &Pubkey) -> Option<usize> {
    vec.iter().position(|x| pubkey_eq(x, target))
}

#[profile]
pub fn update_index(vec: &mut Vec<Pubkey, 10>, index: usize, new_value: &Pubkey) {
    vec[index] = *new_value;
}

#[profile]
pub fn update_get_mut(
    vec: &mut Vec<Pubkey, 10>,
    index: usize,
    new_value: &Pubkey,
) -> Result<(), ProgramError> {
